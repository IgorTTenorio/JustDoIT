'''
---------------------------------------------------------------------------------------------------
    VERSÃO 1.0.0 - MÓDULO GERENTE LIXEIRA INTELIGENTE
    
    Elaborador: Igor Tenório Teixeira                                        Data: 28/03/19
    
    Conteúdo: Nesta versão do módulo gerente será testado a coleta de informações no protótipo
    inicial da lixeira. Os valores coletados da Base_1.0 através de comunicação serial serão
    exibidos conforme o número de medidas escolhido, ao escolher o comando sair um arquivo XLS
    será criado.
---------------------------------------------------------------------------------------------------
'''
      ...
      if Opcao == "1":
         ...
         for j in range(int(Total_de_Medidas)):
            ...
            if len(PacoteRX) == 52:
               ...
               #ad2h = PacoteRX[23] # inteiro
               #ad2l = PacoteRX[24] # resto

               #ad3h = PacoteRX[26] # inteiro
               #ad3l = PacoteRX[27] # resto

               # Converte o número recebido para o valor multiplicado por 256 e divide por 100
               Dist_M = (ad0h * 256 + ad0l)/100.0
               Dist_C = (ad1h * 256 + ad1l)/100.0
           
            print ('Medida = ',j+1,'     Distância em Metros = '     ,Dist_M, ' m','     Distância em Centímetros = '     ,Dist_C,' cm')     
            ws.write(j+2,0,j+1)
            ws.write(j+2,1,Dist_M)
            ws.write(j+2,2,Dist_C)
            time.sleep(2)
